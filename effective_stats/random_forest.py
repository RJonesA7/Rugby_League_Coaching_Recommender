...
And trains a random forest regressor using sklearn to predict matches
"""
def rf_scikit(weights):
    ...
    #Param grid to search over, change to fit speed or search quality choices
    param_grid = {
        # keep trees modest while tuning
        "n_estimators": [150, 300],

        # cap complexity for small data
        "max_depth": [10, 20, None],

        # strongest regulariser here
        "min_samples_leaf": [3, 8],

        # classic default + one “wider” option
    ...
    # Drop zero-variance columns on test to match train columns is better done by aligning columns:
    X_test = X_test.fillna(0)
    X_test = X_test.reindex(columns=X.columns, fill_value=0)  #ensure same columns and order as training
